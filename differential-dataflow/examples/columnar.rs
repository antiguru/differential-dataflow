...
pub mod batcher {
    ...
    pub type Col2ValBatcher<K, V, T, R> = MergeBatcher<Column<((K,V),T,R)>, Chunker<ColumnBuilder<((K,V),T,R)>>, merger::ColumnMerger<(K,V),T,R>>;
    ...
    pub mod merger {
        ...
        pub type ColumnMerger<D, T, R> = ContainerMerger<ColumnBuilder<(D,T,R)>,ColumnQueue<(D, T, R)>>;
