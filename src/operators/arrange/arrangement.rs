    ...
    T1: TraceReader + Clone + 'static,
{
    ...
    where
        T2: for<'a> TraceReader<Key<'a>=T1::Key<'a>,TimeOwned=T1::TimeOwned, Time<'a>=T1::Time<'a>>+Clone+'static,
    ...
    T1: TraceReader + Clone + 'static,
{
    ...
    where
        T2: for<'a> Trace<Key<'a>=T1::Key<'a>, TimeOwned=T1::TimeOwned, Time<'a>=T1::Time<'a>>+'static,
        V: Data,
        F: Fn(T2::Val<'_>) -> V + 'static,
        T2::Batch: Batch,
        T2::Builder: Builder<Input = ((T1::KeyOwned,V), T2::TimeOwned, T2::DiffOwned)>,
