...
pub trait ReduceCore<G: Scope, K: ToOwned + ?Sized, V: ToOwned + ?Sized, R: Semigroup> where G::Timestamp: Lattice+Ord {
    ...
    fn reduce_abelian<L, T2>(&self, name: &str, mut logic: L) -> Arranged<G, TraceAgent<T2>>
        where
            ...
            T2::Diff: Abelian,
            T2::Batch: Batch,
            T2::Builder: Builder<InputBatch=Vec<((K::Owned, T2::ValOwned), T2::Time, T2::Diff)>, Input = ((K::Owned, T2::ValOwned), T2::Time, T2::Diff)>,
...
where
    ...
    T2::ValOwned: Data,
    T2::Batch: Batch,
    T2::Builder: Builder<InputBatch=Vec<((T1::KeyOwned, T2::ValOwned), T2::Time, T2::Diff)>, Input = ((T1::KeyOwned, T2::ValOwned), T2::Time, T2::Diff)>,
