...
mod val_batch {
    ...
    impl<L: Layout> OrdValMerger<L> {
        ...
        fn stash_updates_for_val(&mut self, source: &OrdValStorage<L>, index: usize) {
            ...
            for i in lower .. upper {
                ...
                let time = &source.times.index(i).into_owned();
        ...
        fn consolidate_updates(&mut self) -> Option<usize> {
            ...
            if !self.update_stash.is_empty() {
                ...
                if self.update_stash.len() == 1 && self.result.times.last().zip(self.result.diffs.last()).map(|(time, diff)| {
                    let (last_time, last_diff) = self.update_stash.last().unwrap();
                    time.equals(last_time) && diff.equals(last_diff)
                }).unwrap_or(false) {
    ...
    impl<L: Layout> Cursor for OrdValCursor<L> {
        ...
        fn map_times<L2: FnMut(&Self::TimeOwned, Self::Diff<'_>)>(&mut self, storage: &OrdValBatch<L>, mut logic: L2) {
            ...
            let mut time = Self::TimeOwned::minimum();
            for index in lower .. upper {
                storage.storage.times.index(index).clone_onto(&mut time);
    ...
    impl<L: Layout> OrdValBuilder<L> {
        ...
        fn push_update(&mut self, time: <L::Target as Update>::Time, diff: <L::Target as Update>::Diff) {
            ...
            if self.result.times.last().zip(self.result.diffs.last()).map(|(t, d)| t.equals(&time) && d.equals(&diff)) == Some(true) {
    ...
    impl<L: Layout> OrdKeyMerger<L> {
        ...
        fn consolidate_updates(&mut self) -> Option<usize> {
            ...
            if !self.update_stash.is_empty() {
                // If there is a single element, equal to a just-prior recorded update,
                // we push nothing and report an unincremented offset to encode this case.
                if self.update_stash.len() == 1 && self.update_stash.last().zip(self.result.times.last()).zip(self.result.diffs.last()).map_or(false, |(((last_time, last_diff), time), diff)| time.equals(last_time) && diff.equals(last_diff)) {
                    // Just clear out update_stash, as we won't drain it here.
                    self.update_stash.clear();
                    self.singletons += 1;
                }
